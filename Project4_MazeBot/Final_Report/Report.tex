...
\subsection{Constants Used to Represent the Size of the Maze}
\begin{itemize}
\item Declared a constant that represents the dimension of a single cell
	\begin{verbatim}
		float const SIZE_OF_ONE_CELL = 22.5425; // in cm 	
	\end{verbatim} 
\item Declared four constants that represent the size of the maze
	\begin{verbatim}
...
\begin{itemize}
\item Declared two constants that represent the initial position of the robot in the maze. This will be entered when we begin our demo.
	\begin{verbatim}
		int const START_ROW = ;
		int const START_COL = ;
	\end{verbatim} 
\item Declared two constants that represents the target position in the maze. This will be entered when we begin our demo.
	\begin{verbatim}
		int const END_ROW = ; 
		int const END_COL = ;
	\end{verbatim}
\item Declared two variables that represents the current position of the robot in the maze. This is  initialized as the starting position. 
	\begin{verbatim}
		int currentRow = START_ROW;
		int currentCol = START_COL;
	\end{verbatim} 
\item Declared an array that represents the orientation that the bot has as it enters each cell. The size of the array is four times larger than the multiple of maze width and maze height because the maximum amount of times that the robot can go into each cell is four times.
	\begin{verbatim}
...
\begin{itemize}
\item The four constants were declared to represent each directions. 
	\begin{verbatim}
		#define NORTH 0
		#define EAST 1
		#define SOUTH 2
...
\item Variable name "Maze" was declared which has the data type cell. This data type includes five parameters: North wall, South wall, East wall, West wall, and Visited (used to tell if the robot has visited the cell or not) as discussed above.
	\begin{verbatim}
		cell Maze[MAZE_HEIGHT][MAZE_WIDTH];
	\end{verbatim}
\item Global variable which represents current direction of the robot was declared. This is initialized as north as this is the orientation of the robot when it first enters the maze.
	\begin{verbatim}
		int direction = NORTH;
	\end{verbatim}
\end{itemize}

\subsection{Variables Used for Moving Mechanism}
\begin{itemize}
\item The uncertainty of two motors and backlash of the gears cause the robot not to move in exact distance required. Therefore, three constants were declared which are added to the encoder input values. 
	\begin{verbatim}
...
\item Each speed of the motors were defined with constants to simplify the code. 
	...
	\end{verbatim} 
\item Encoder input constants are declared
	\begin{verbatim}
